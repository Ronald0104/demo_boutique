...
$(function () {
	...
	$('#btn-add-article-full').on('click', function (evt) {
		...
		pet.done(function (data) {
			if (data) {
				...
				fn_AgregarItemStorage(item.codigo);
				fn_Enumerar();
				fn_CalcularTotal();
				$('#codigoArticulo').val('');	
			} else {
	...
	$('#btn-show-search-article').on('click', (evt) => {
		evt.preventDefault();
	});
	...
	$('#totalDescuento').on('keyup', function () {
		fn_CalcularTotal();
	})
	...
	$('#nroDocumento').on('keydown', function (evt) {
		...
		.on('keypress', function (evt) {
			...
			if (keyCode == 13) {
				...
				// fn_ObtenerHistorialCliente($(this).val());
			}
			// return false;
	...
	function fn_ObtenerHistorialCliente() {
		// if ($('#nroDocumento').data('valueOld') == $('#nroDocumento').val().trim()) return;

		$.ajax({
			type: 'POST',
			url: '/ventas/listarVentasPorDocumentoCliente',
			data: { numero: $('#nroDocumento').val(), cantidad: 10 },
			beforeSend: Call_Progress(true),
			//contentType: 'application/json; charset=utf-8'
			dataType: 'json'
		})
		.done(function(data) {
			// LLenar la tabla de historial del cliente
			if (data.length == 0) {
				$('#alert-history-client-empty').removeClass("d-none");
				$('#table-history-client').addClass("d-none");
			} else {
				$('#alert-history-client-empty').addClass("d-none");
				$('#table-history-client').removeClass("d-none");
				
				data.forEach((e, i) => {
					let tr = $("<tr>");
					let fila = `<tr>
						<td>${i+1}</td>
						<td><a href="/ventas/editar/${e.ventaId}" target="_blank">${e.numeroOperacion}</a></td>
						<td>Tienda ${e.tiendaId}</td>
						<td>${e.fechaRegistro}</td>
						<td>${e.fechaDevolucion}</td>
						<td>${e.precioTotal}</td>
						<td>${e.totalPagado}</td>
						<td>${e.estado}</td>
						<td>${e.vendedor}</td>
						<td class="text-center"><button class="btn btn-sm btn-blue btn-item-show" data-item="${e.ventaId}" onclick="javascript:window.open('/ventas/editar/${e.ventaId}');return;"><i class="icon icon-pencil5"></i></button></td>
						</tr>
						`;
					$('#table-history-client').append(fila);					
				})
			}
	...
	$('#btn-register-sale').on('click', function (evt) {
		...
		if ($('#tipoOperacion').val() == 1){
			if ($('#estado').val() == 1) {	// RESERVADO
				...
				let descuento = $("#totalDescuento").val();
				let pagado = $("#totalPagado").val();

				if (total - descuento > 0) {
					if (pagado <= 0) { // Debe haber un minimo de pago por la reserva
...
function fn_GenerarVenta() {
	...
	sale.totalDescuento = Number($('#totalDescuento').val());
...
function fn_LimpiarHistorial() {
	$("#table-history-client > tbody").html("");
}
...
function fn_CalcularTotal() {
	...
	let descuento = Number($('#totalDescuento').val());
	pagado = efectivo + tarjeta - vuelto;
	saldo = total - pagado - descuento;
...
function fn_Inicializar() {
	...
	if (storageItems.length > 0) {
		// Agregar articulo
		storageItems.forEach((e, i) => {
			console.log(e);
			fn_ObtenerArticulo(e);
		})
	}
}

function fn_ObtenerArticulo(codigo) {
	var pet = $.ajax({
		method: 'POST',
		url: '/inventario/articuloByCodeFull',
		data: { articuloCode: codigo },
...
function fn_InicializarStorage() {
	if(!sessionStorage.getItem("sale-items")) { sessionStorage.setItem("sale-items", "[]") };
	storageItems = JSON.parse(sessionStorage.getItem("sale-items"));
	// console.log('storage Items: ', storageItems);
}

function fn_AgregarItemStorage(codigo) {
	storageItems.push(codigo);
	sessionStorage.setItem("sale-items", JSON.stringify(storageItems));
}

function fn_EliminarItemStorage(codigo) {
	let idx = storageItems.findIndex(p => p == codigo)
	storageItems.splice(idx, 1);
	sessionStorage.setItem("sale-items", JSON.stringify(storageItems));
}

function fn_LimpiarStorage() {
	sessionStorage.setItem("sale-items", "[]")
}