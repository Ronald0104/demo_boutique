...
$(function () {
	...
	// Colocarlo como un componente compartido
	...
	$('#descripcion-producto').on('keyup', function (evt) {
		...
		$.ajax({
			...
			data: { descripcion: $('#descripcion-producto').val() },
			dataType: 'json'
		})
			.done(function (data) {
				fn_mostrarArticulos(data);
				...
				if (articleReserves.length && codigo.substr(0, 3) != "GEN") {
					...
					if (!fechaSalida || !fechaDevolucion) {
						...
						articleReserves.forEach(function (e) {
							...
							// console.log(moment(fechaSalida));							
...
function fn_AgregarArticulo(item) {	
	let tr, input, a, div;
	tr = $('<tr>').data('articulo', item.id).data('tipo', item.tipo).prop('tabindex', 0);;
	tr.append($('<td>').text(0));
